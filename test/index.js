...
describe('public key only wallet', function () {
  ...
  it('.fromPublicKey() should work', function () {
    assert.equal(Wallet.fromPublicKey(pubKey).getPublicKey().toString('hex'), fixturePublicKey);
  });
  ...
  it('.fromPublicKey() should accept compressed keys in non-strict mode', function () {
    ...
    assert.equal(Wallet.fromPublicKey(tmp, true).getPublicKey().toString('hex'), '0639797f6cc72aea0f3d309730844a9e67d9f1866e55845c5f7e0ab48402973defa5cb69df462bcc6d73c31e1c663c225650e80ef14a507b203f2a12aea55bc1');
  });
...
describe('.generate()', function () {
  ...
  it('should generate an account compatible with ICAP Direct', function () {
    ...
    assert.equal(new yggUtil.BN(wallet.getAddress()).lte(max), true);
  });
});

...
Wallet.generateVanityAddress = function (pattern) {
  ...
  while (true) {
    var privKey = crypto.randomBytes(32);
    var address = yggUtil.privateToAddress(privKey);
...
Wallet.prototype.toV3 = function (password, opts) {
  ...
  var mac = yggUtil.sha3(Buffer.concat([derivedKey.slice(16, 32), Buffer.from(ciphertext, 'hex')]));
...
Wallet.fromV1 = function (input, password) {
  ...
  var mac = yggUtil.sha3(Buffer.concat([derivedKey.slice(16, 32), ciphertext]));

  if (mac.toString('hex') !== json.Crypto.MAC) {
    throw new Error('Key derivation failed - possibly wrong passphrase');
  }

  var decipher = crypto.createDecipheriv('aes-128-cbc', yggUtil.sha3(derivedKey.slice(0, 16)).slice(0, 16), Buffer.from(json.Crypto.IV, 'hex'));
...
Wallet.fromEthSale = function (input, password) {
  ...
  var wallet = new Wallet(yggUtil.sha3(seed));
